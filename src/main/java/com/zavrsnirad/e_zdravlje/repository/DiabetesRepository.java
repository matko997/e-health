...
public interface DiabetesRepository extends JpaRepository<Diabetes, Long> {
  ...
  @Query(
      "SELECT d FROM Diabetes d JOIN d.patient WHERE d.patient.firstName LIKE %:keyword% OR d.patient.lastName "
          + "LIKE %:keyword% ")
  Page<Diabetes> findDiabetesPaginatedAndFilterable(
      Pageable pageable, @Param("keyword") String keyword);

  @Query(
      "SELECT d FROM Diabetes d JOIN d.patient WHERE d.patient=:patient AND (d.patient.firstName "
          + "LIKE %:keyword% OR d.patient.lastName LIKE %:keyword% ) ")
  Page<Diabetes> findAllByPatientFilterable(
  ...
  long getCountOfDiabetesBetween(
