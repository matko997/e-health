...
public interface VaccineRepository extends JpaRepository<Vaccine, Long> {
  ...
  @Query(
      "SELECT v FROM Vaccine v JOIN v.doctor JOIN v.patient WHERE v.name LIKE %:keyword% OR v.doctor.firstName "
          + "LIKE %:keyword% OR v.doctor.lastName LIKE %:keyword% OR v.patient.firstName LIKE %:keyword% "
          + "OR v.patient.lastName LIKE %:keyword% ")
  Page<Vaccine> findVaccinePaginatedAndFilterable(
  ...
  long getCountOfVaccinesBetween(
