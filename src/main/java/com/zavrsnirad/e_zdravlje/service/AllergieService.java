...
public class AllergieService {
    ...
    public Paged<Allergie> findAllergiesPaginatedFilterable(int pageNumber, int size, String keyword) {
        PageRequest request = PageRequest.of(pageNumber - 1, size);
        Page<Allergie> vaccinePage = allergiesRepository.findAllergiesPaginatedAndFilterable(request, keyword);
    ...
    public Paged<Allergie> findAllergiesPaginatedAndFilterableForPatient(int pageNumber, int size, User user, String keyword) {
        PageRequest request = PageRequest.of(pageNumber - 1, size);
        Page<Allergie> allergiePage = allergiesRepository.findAllByPatientFilterable(user, request, keyword);
        return new Paged<>(allergiePage, Paging.of(allergiePage.getTotalPages(), pageNumber, size));
    }
