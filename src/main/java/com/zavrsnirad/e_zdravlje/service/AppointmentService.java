...
public class AppointmentService {
  ...
  public Paged<Appointment> findAppointmentsPaginated(int pageNumber, int size) {
    ...
    return new Paged<>(
        appointmentPage, Paging.of(appointmentPage.getTotalPages(), pageNumber, size));
  }

  public Paged<Appointment> findAppointmentsPaginatedFilterable(
      int pageNumber, int size, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<Appointment> appointmentPage =
        appointmentRepository.findAppointmentPaginatedAndFilterable(request, keyword);
    return new Paged<>(
        appointmentPage, Paging.of(appointmentPage.getTotalPages(), pageNumber, size));
  }

  public Paged<Appointment> findAppointmentsPaginatedForPatient(
  ...
  public Paged<Appointment> findAppointmentsPaginatedAndFilterableForPatient(
      int pageNumber, int size, User user, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<Appointment> appointmentPage =
        appointmentRepository.findAllByPatientFilterable(user, request, keyword);
    return new Paged<>(
        appointmentPage, Paging.of(appointmentPage.getTotalPages(), pageNumber, size));
  }
