...
public class BloodPressureService {
  ...
  public BloodPressureService(
      BloodPressureRepository bloodPressureRepository, UserRepository userRepository) {
  ...
  public Paged<BloodPressure> findBloodPressuresPaginated(int pageNumber, int size) {
    ...
    return new Paged<>(
        bloodPressurePage, Paging.of(bloodPressurePage.getTotalPages(), pageNumber, size));
  }

  public Paged<BloodPressure> findBloodPressuresPaginatedFilterable(
      int pageNumber, int size, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<BloodPressure> bloodPressurePage =
        bloodPressureRepository.findBloodPressuresPaginatedAndFilterable(request, keyword);
    return new Paged<>(
        bloodPressurePage, Paging.of(bloodPressurePage.getTotalPages(), pageNumber, size));
  }

  public Paged<BloodPressure> findBloodPressuresPaginatedForPatient(
      int pageNumber, int size, User user) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
  ...
  public Paged<BloodPressure> findBloodPressuresPaginatedAndFilterableForPatient(
      int pageNumber, int size, User user, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<BloodPressure> bloodPressurePage =
        bloodPressureRepository.findAllByPatientFilterable(user, request, keyword);
    return new Paged<>(
        bloodPressurePage, Paging.of(bloodPressurePage.getTotalPages(), pageNumber, size));
  }
  ...
  public void editBloodPressure(BloodPressure bloodPressure) {
    Optional<BloodPressure> bloodPressureOptional =
        bloodPressureRepository.findById(bloodPressure.getId());
