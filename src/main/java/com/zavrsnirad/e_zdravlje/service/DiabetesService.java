...
public class DiabetesService {
    ...
    public Paged<Diabetes> findDiabetesPaginatedFilterable(int pageNumber, int size, String keyword) {
        PageRequest request = PageRequest.of(pageNumber - 1, size);
        Page<Diabetes> diabetesPage = diabetesRepository.findDiabetesPaginatedAndFilterable(request, keyword);
        return new Paged<>(diabetesPage, Paging.of(diabetesPage.getTotalPages(), pageNumber, size));
    }
    ...
    public Paged<Diabetes> findDiabetesPaginatedAndFilterableForPatient(int pageNumber, int size, User user, String keyword) {
        PageRequest request = PageRequest.of(pageNumber - 1, size);
        Page<Diabetes> diabetesPage = diabetesRepository.findAllByPatientFilterable(user, request, keyword);
        return new Paged<>(diabetesPage, Paging.of(diabetesPage.getTotalPages(), pageNumber, size));
    }
