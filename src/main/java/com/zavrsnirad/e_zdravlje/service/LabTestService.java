...
public class LabTestService {
  ...
  public Paged<LabTest> findLabTestsPaginatedFilterable(int pageNumber, int size, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<LabTest> labTestPage =
        labTestRepository.findLabTestPaginatedAndFilterable(request, keyword);
    return new Paged<>(labTestPage, Paging.of(labTestPage.getTotalPages(), pageNumber, size));
  }
  ...
  public Paged<LabTest> findLabTestsPaginatedAndFilterableForPatient(
      int pageNumber, int size, User user, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<LabTest> labTestPage =
        labTestRepository.findAllByPatientFilterable(user, request, keyword);
    return new Paged<>(labTestPage, Paging.of(labTestPage.getTotalPages(), pageNumber, size));
  }
