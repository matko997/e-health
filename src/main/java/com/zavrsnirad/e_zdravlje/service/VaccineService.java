...
public class VaccineService {
  ...
  public Paged<Vaccine> findVaccinePaginatedAndFilterableForPatient(
      int pageNumber, int size, User user, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<Vaccine> vaccinePage =
        vaccineRepository.findAllByPatientFilterable(user, request, keyword);
    return new Paged<>(vaccinePage, Paging.of(vaccinePage.getTotalPages(), pageNumber, size));
  }
  ...
  public Paged<Vaccine> findVaccinePaginatedAndFiltered(int pageNumber, int size, String keyword) {
    PageRequest request = PageRequest.of(pageNumber - 1, size);
    Page<Vaccine> vaccinePage =
        vaccineRepository.findVaccinePaginatedAndFilterable(request, keyword);
    return new Paged<>(vaccinePage, Paging.of(vaccinePage.getTotalPages(), pageNumber, size));
  }
